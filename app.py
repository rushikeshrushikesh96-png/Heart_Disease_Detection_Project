...
# ===============================================================
    ...
    report = f"""HEART DISEASE RISK REPORT
{'=' * 50}
Generated by the Heart Disease Detection AI project

PATIENT
  Age                 : {PATIENT['age']:.0f} years
  Gender               : {'Male' if PATIENT['gender'] else 'Female'}
  Height / Weight      : {PATIENT['height']} cm / {PATIENT['weight']} kg
  BMI                  : {bmi_v}
  Blood pressure       : {PATIENT['ap_hi']}/{PATIENT['ap_lo']} mmHg
  Pulse pressure       : {PATIENT['ap_hi'] - PATIENT['ap_lo']} mmHg
  Cholesterol level    : {PATIENT['cholesterol']} ({['Normal','Above normal','Well above normal'][PATIENT['cholesterol']-1]})
  Glucose level        : {PATIENT['gluc']} ({['Normal','Above normal','Well above normal'][PATIENT['gluc']-1]})
...
MODEL PREDICTIONS
  Logistic Regression  : {vals['Logistic Regression']*100:.1f}%
  Random Forest        : {vals['Random Forest']*100:.1f}%
  SVM                  : {vals['SVM']*100:.1f}%
  ------------------------------------------------
  AVERAGE              : {avg*100:.1f}%
  Model agreement       : within {spread*100:.1f} percentage points
...
{'-' * 50}
DISCLAIMER: This is an academic machine-learning project for educational
purposes only. It is not a medical device, has not been clinically
validated, and must never be used to make real health decisions.
Always consult a qualified doctor.
"""
    st.download_button("⬇️ Download patient report (.txt)", report,
                       f"heart_risk_report_{PATIENT['age']:.0f}yo.txt", "text/plain")
